...
#
q_glue_vp_fdc      = np.sort(x_q_glue_vp.T, axis = 0)[::-1] #(5114, 1000)
q_glue_best_vp_fdc = np.sort(x_q_glue_vp[640,:])[::-1] #(5114,)
...
print('npkge min, max, mean, std = ', '{0:.2g}'.format(np.min(metrics_dds_vp_list[:,1])), \
        ...
        '{0:.2g}'.format(np.std(metrics_dds_vp_list[:,1]))) #npkge min, max, mean, std =  0.026 0.8 0.68 0.062
...
#
q_dds_vp_fdc      = np.sort(x_q_dds_vp.T, axis = 0)[::-1] #(5114, 1000)
q_dds_best_vp_fdc = np.sort(x_q_dds_vp[4621,:])[::-1] #(5114,)
...
#
q_dl_vp_fdc      = np.sort(x_q_dl_vp.T, axis = 0)[::-1] #(5114, 1000)
q_dl_best_vp_fdc = np.sort(x_q_dl_vp[2881,:])[::-1] #(5114,)
...
#
q_glue_vp_err_list   = np.zeros((len(vp_t_list),num_glue_realz), dtype = float) #Percent error list for glue; #(5114, 25050)
...
q_glue_vp_mean_variation = (1.0/len(vp_t_list)) * np.sum(q_glue_vp_lbub_list[:,2]) #32.171302998435664
q_glue_vp_prob           = (1.0/len(vp_t_list)) * np.sum(q_glue_vp_uncer_list) #0.990418459131795
...
#
q_dds_vp_err_list   = np.zeros((len(vp_t_list),num_dds_realz), dtype = float) #Percent error list for dds; #(5114, 25050)
...
q_dds_vp_mean_variation = (1.0/len(vp_t_list)) * np.sum(q_dds_vp_lbub_list[:,2]) #26.406462380572936
q_dds_vp_prob           = (1.0/len(vp_t_list)) * np.sum(q_dds_vp_uncer_list) #0.9892452092295658
...
#
q_dl_vp_err_list   = np.zeros((len(vp_t_list),num_dl_realz), dtype = float) #Percent error list for dl; #(5114, 25050)
...
q_dl_vp_mean_variation = (1.0/len(vp_t_list)) * np.sum(q_dl_vp_lbub_list[:,2]) #14.740415390594444
q_dl_vp_prob           = (1.0/len(vp_t_list)) * np.sum(q_dl_vp_uncer_list) #0.9890496675791943
...
str_x_label    = 'Time [Days]'
str_y_label    = r'Streamflow [$\frac{m^3}{s}$]'
fl_name        = path + 'Plots/SWAT_vs_Obs/GLUE_vs_Obs_q_vp_1' 
legend_loc     = 'upper left'
obs_label_list = ['Observed', 'Calibrated SWAT (GLUE)']
num_ticks      = 800
line_width_ens = [1.0, 1.0]
...
str_x_label    = 'Time [Days]'
str_y_label    = r'Streamflow [$\frac{m^3}{s}$]'
fl_name        = path + 'Plots/SWAT_vs_Obs/DDS_vs_Obs_q_vp_1' 
legend_loc     = 'upper left'
obs_label_list = ['Observed', 'Calibrated SWAT (DDS)']
num_ticks      = 800
line_width_ens = [1.0, 1.0]
...
str_x_label    = 'Time [Days]'
str_y_label    = r'Streamflow [$\frac{m^3}{s}$]'
fl_name        = path + 'Plots/SWAT_vs_Obs/CNN_vs_Obs_q_vp_1' 
legend_loc     = 'upper left'
obs_label_list = ['Observed', 'Calibrated SWAT (CNN)']
num_ticks      = 800
line_width_ens = [1.0, 1.0]
...
str_x_label     = r'Flow exceedence [\%]'
str_y_label     = r'Streamflow [$\frac{m^3}{s}$]'
fig_name        = path + 'Plots/SWAT_vs_Obs/FDC_GLUE_vs_Obs_q_vp_1' 
legend_loc      = 'lower left'
plot_label_list = ['Observed', 'Best calibration set (GLUE)', 'All GLUE sets']
num_ticks       = 10
line_width_ens  = [1.0, 2.0, 0.12]
...
str_x_label     = r'Flow exceedence [\%]'
str_y_label     = r'Streamflow [$\frac{m^3}{s}$]'
fig_name        = path + 'Plots/SWAT_vs_Obs/FDC_DDS_vs_Obs_q_vp_1' 
legend_loc      = 'lower left'
plot_label_list = ['Observed', 'Best calibration set (DDS)', 'All DDS sets']
num_ticks       = 10
line_width_ens  = [1.0, 2.0, 0.12]
...
str_x_label     = r'Flow exceedence [\%]'
str_y_label     = r'Streamflow [$\frac{m^3}{s}$]'
fig_name        = path + 'Plots/SWAT_vs_Obs/FDC_CNN_vs_Obs_q_vp_1' 
legend_loc      = 'lower left'
plot_label_list = ['Observed', 'Best calibration set (CNN)', 'All CNN sets']
num_ticks       = 10
line_width_ens  = [1.0, 2.0, 0.12]
...
#
q_glue_mv_vtp   = 32.171302998435664 #Size of mean variation for validation time period (glue)
q_dds_mv_vtp    = 26.406462380572936 #Size of mean variation for validation time period (dds)
q_dl_mv_vtp     = 14.740415390594444  #Size of mean variation for validation time period (dl)
#
q_glue_prob_vtp = 0.990418459131795 #Prob that obs. data is contained within the lb and ub boundary (glue)
q_dds_prob_vtp  = 0.9892452092295658 #Prob that obs. data is contained within the lb and ub boundary (dds)
q_dl_prob_vtp   = 0.9890496675791943 #Prob that obs. data is contained within the lb and ub boundary (dl)
#
...
fig_name        = path + 'Plots/SWAT_vs_Obs/Size_MV_GLUE_DDS_CNN_all1'
...
width_val       = 0.15
loc_pos         = 'upper center'
fig_name        = path + 'Plots/SWAT_vs_Obs/Size_Prob_GLUE_DDS_CNN_all1'
