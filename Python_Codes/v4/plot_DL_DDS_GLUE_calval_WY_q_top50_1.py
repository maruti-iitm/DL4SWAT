...
#   1b. Validation Period  --> Water Year (WY) from 1999 to 2013 -- Oct-1-1999 to Sep-30-2013 -- .iloc[1003:6117,:]
...
temp_glue_vp = np.transpose(df_q_glue_vp.iloc[:,1:].values + 0.00001)[0:1000,:] #Matrix of values of discharge -- 1000 realz; (1000, 5114)
#
x_q_glue_cp  = copy.deepcopy(temp_glue_cp[df_argsm_glue_cp.iloc[0:50,4].values,:]) #(50, 1096)
x_q_glue_vp  = copy.deepcopy(temp_glue_vp[df_argsm_glue_vp.iloc[0:50,4].values,:]) #(50, 5114)
#
q_glue_cp_fdc      = np.sort(x_q_glue_cp.T, axis = 0)[::-1] #(1096, 50)
q_glue_best_cp_fdc = np.sort(x_q_glue_cp[0,:])[::-1] #(1096,)
#
q_glue_vp_fdc      = np.sort(x_q_glue_vp.T, axis = 0)[::-1] #(5114, 50)
q_glue_best_vp_fdc = np.sort(x_q_glue_vp[0,:])[::-1] #(5114,)
...
temp_dds_vp  = np.transpose(df_q_dds_vp.iloc[:,1:].values + 0.00001) #Matrix of values of discharge -- 5000 realz; (5000, 5114)
#
x_q_dds_cp   = copy.deepcopy(temp_dds_cp[df_argsm_dds_cp.iloc[0:50,4].values,:]) #(50, 1096)
x_q_dds_vp   = copy.deepcopy(temp_dds_vp[df_argsm_dds_vp.iloc[0:50,4].values,:]) #(50, 5114)
#
q_dds_cp_fdc      = np.sort(x_q_dds_cp.T, axis = 0)[::-1] #(1096, 50)
q_dds_best_cp_fdc = np.sort(x_q_dds_cp[0,:])[::-1] #(1096,)
#
q_dds_vp_fdc      = np.sort(x_q_dds_vp.T, axis = 0)[::-1] #(5114, 50)
q_dds_best_vp_fdc = np.sort(x_q_dds_vp[0,:])[::-1] #(5114,)
...
temp_dl_vp   = np.concatenate((x_q_vp1, x_q_vp2)) #(25050, 5114)
#
x_q_dl_cp    = copy.deepcopy(temp_dl_cp[df_argsm_dl_cp.iloc[0:50,4].values,:]) #(50, 1096)
x_q_dl_vp    = copy.deepcopy(temp_dl_vp[df_argsm_dl_vp.iloc[0:50,4].values,:]) #(50, 5114)
#
q_dl_cp_fdc      = np.sort(x_q_dl_cp.T, axis = 0)[::-1] #(1096, 50)
q_dl_best_cp_fdc = np.sort(x_q_dl_cp[0,:])[::-1] #(1096,)
#
q_dl_vp_fdc      = np.sort(x_q_dl_vp.T, axis = 0)[::-1] #(5114, 50)
q_dl_best_vp_fdc = np.sort(x_q_dl_vp[0,:])[::-1] #(5114,)
...
#
q_glue_vp_err_list   = np.zeros((len(vp_t_list),num_glue_realz), dtype = float) #Percent error list for glue; #(5114, 50)
q_glue_vp_uncer_list = np.zeros(len(vp_t_list), dtype = int) #Obs data within the uncertainity boundary (glue) #(5114,)
q_glue_vp_lbub_list  = np.zeros((len(vp_t_list),3), dtype = float) #lbub list for glue #(5114, 3)
#
for i in range(0,num_glue_realz):
...
q_glue_vp_mean_variation = (1.0/len(vp_t_list)) * np.sum(q_glue_vp_lbub_list[:,2]) #12.880552600703949
q_glue_vp_prob           = (1.0/len(vp_t_list)) * np.sum(q_glue_vp_uncer_list) #0.8844348846304262
...
#
q_dds_vp_err_list   = np.zeros((len(vp_t_list),num_dds_realz), dtype = float) #Percent error list for dds; #(5114, 50)
q_dds_vp_uncer_list = np.zeros(len(vp_t_list), dtype = int) #Obs data within the uncertainity boundary (dds) #(5114,)
q_dds_vp_lbub_list  = np.zeros((len(vp_t_list),3), dtype = float) #lbub list for dds #(5114, 3)
#
for i in range(0,num_dds_realz):
...
q_dds_vp_mean_variation = (1.0/len(vp_t_list)) * np.sum(q_dds_vp_lbub_list[:,2]) #6.624746304262807
q_dds_vp_prob           = (1.0/len(vp_t_list)) * np.sum(q_dds_vp_uncer_list) #0.6609307782557684
...
#
q_dl_vp_err_list   = np.zeros((len(vp_t_list),num_dl_realz), dtype = float) #Percent error list for dl; #(5114, 50)
q_dl_vp_uncer_list = np.zeros(len(vp_t_list), dtype = int) #Obs data within the uncertainity boundary (dl) #(5114,)
q_dl_vp_lbub_list  = np.zeros((len(vp_t_list),3), dtype = float) #lbub list for dl #(5114, 3)
#
for i in range(0,num_dl_realz):
...
q_dl_vp_mean_variation = (1.0/len(vp_t_list)) * np.sum(q_dl_vp_lbub_list[:,2]) #4.560889753617521
q_dl_vp_prob           = (1.0/len(vp_t_list)) * np.sum(q_dl_vp_uncer_list) #0.5891669925694173
...
#
q_glue_vp_lb_list = x_q_glue_vp.min(axis = 0) #(5114,)
q_glue_vp_ub_list = x_q_glue_vp.max(axis = 0) #(5114,)
#
q_dds_cp_lb_list  = x_q_dds_cp.min(axis = 0) #(1096,)
q_dds_cp_ub_list  = x_q_dds_cp.max(axis = 0) #(1096,)
#
q_dds_vp_lb_list  = x_q_dds_vp.min(axis = 0) #(5114,)
q_dds_vp_ub_list  = x_q_dds_vp.max(axis = 0) #(5114,)
#
q_dl_cp_lb_list   = x_q_dl_cp.min(axis = 0) #(1096,)
q_dl_cp_ub_list   = x_q_dl_cp.max(axis = 0) #(1096,)
#
q_dl_vp_lb_list   = x_q_dl_vp.min(axis = 0) #(5114,)
q_dl_vp_ub_list   = x_q_dl_vp.max(axis = 0) #(5114,)
...
fl_name        = path + 'Plots/SWAT_vs_Obs/GLUE_vs_Obs_q_cp_v2' 
...
str_x_label    = 'Time [Days]'
str_y_label    = r'Streamflow [$\frac{m^3}{s}$]'
fl_name        = path + 'Plots/SWAT_vs_Obs/GLUE_vs_Obs_q_vp_v2' 
legend_loc     = 'upper left'
obs_label_list = ['Observed', 'Calibrated SWAT (GLUE)']
num_ticks      = 800
line_width_ens = [1.0, 1.5]
...
str_x_label    = 'Time [Days]'
str_y_label    = r'Streamflow [$\frac{m^3}{s}$]'
fl_name        = path + 'Plots/SWAT_vs_Obs/DDS_vs_Obs_q_cp_v2' 
legend_loc     = 'upper left'
...
str_x_label    = 'Time [Days]'
str_y_label    = r'Streamflow [$\frac{m^3}{s}$]'
fl_name        = path + 'Plots/SWAT_vs_Obs/DDS_vs_Obs_q_vp_v2' 
legend_loc     = 'upper left'
obs_label_list = ['Observed', 'Calibrated SWAT (DDS)']
num_ticks      = 800
line_width_ens = [1.0, 1.5]
...
str_x_label    = 'Time [Days]'
str_y_label    = r'Streamflow [$\frac{m^3}{s}$]'
fl_name        = path + 'Plots/SWAT_vs_Obs/CNN_vs_Obs_q_cp_v2' 
legend_loc     = 'upper left'
...
str_x_label    = 'Time [Days]'
str_y_label    = r'Streamflow [$\frac{m^3}{s}$]'
fl_name        = path + 'Plots/SWAT_vs_Obs/CNN_vs_Obs_q_vp_v2' 
legend_loc     = 'upper left'
obs_label_list = ['Observed', 'Calibrated SWAT (CNN)']
num_ticks      = 800
line_width_ens = [2.0, 2.0]
...
str_y_label     = r'Streamflow [$\frac{m^3}{s}$]'
fig_name        = path + 'Plots/SWAT_vs_Obs/FDC_GLUE_vs_Obs_q_cp_v2' 
...
str_x_label     = r'Flow exceedence [\%]'
str_y_label     = r'Streamflow [$\frac{m^3}{s}$]'
fig_name        = path + 'Plots/SWAT_vs_Obs/FDC_GLUE_vs_Obs_q_vp_v2' 
legend_loc      = 'lower left'
plot_label_list = ['Observed', 'Best calibration set (GLUE)', 'GLUE sets']
num_ticks       = 10
line_width_ens  = [1.5, 2.0, 0.2]
...
str_x_label     = r'Flow exceedence [\%]'
str_y_label     = r'Streamflow [$\frac{m^3}{s}$]'
fig_name        = path + 'Plots/SWAT_vs_Obs/FDC_DDS_vs_Obs_q_cp_v2' 
legend_loc      = 'lower left'
...
str_x_label     = r'Flow exceedence [\%]'
str_y_label     = r'Streamflow [$\frac{m^3}{s}$]'
fig_name        = path + 'Plots/SWAT_vs_Obs/FDC_DDS_vs_Obs_q_vp_v2' 
legend_loc      = 'lower left'
plot_label_list = ['Observed', 'Best calibration set (DDS)', 'DDS sets']
num_ticks       = 10
line_width_ens  = [1.5, 2.0, 0.2]
...
str_x_label     = r'Flow exceedence [\%]'
str_y_label     = r'Streamflow [$\frac{m^3}{s}$]'
fig_name        = path + 'Plots/SWAT_vs_Obs/FDC_CNN_vs_Obs_q_cp_v2' 
legend_loc      = 'lower left'
...
str_x_label     = r'Flow exceedence [\%]'
str_y_label     = r'Streamflow [$\frac{m^3}{s}$]'
fig_name        = path + 'Plots/SWAT_vs_Obs/FDC_CNN_vs_Obs_q_vp_v2' 
legend_loc      = 'lower left'
plot_label_list = ['Observed', 'Best calibration set (CNN)', 'CNN sets']
num_ticks       = 10
line_width_ens  = [1.5, 2.0, 0.2]
...
#
q_glue_mv_vtp   = 12.880552600703949 #Size of mean variation for validation time period (glue)
q_dds_mv_vtp    = 6.624746304262807 #Size of mean variation for validation time period (dds)
q_dl_mv_vtp     = 4.560889753617521  #Size of mean variation for validation time period (dl)
#
q_glue_prob_vtp = 0.8844348846304262 #Prob that obs. data is contained within the lb and ub boundary (glue)
q_dds_prob_vtp  = 0.6609307782557684 #Prob that obs. data is contained within the lb and ub boundary (dds)
q_dl_prob_vtp   = 0.5891669925694173 #Prob that obs. data is contained within the lb and ub boundary (dl)
#
...
fig_name        = path + 'Plots/SWAT_vs_Obs/Size_MV_GLUE_DDS_CNN_v2'
...
width_val       = 0.15
loc_pos         = 'upper center'
fig_name        = path + 'Plots/SWAT_vs_Obs/Size_Prob_GLUE_DDS_CNN_v2'
